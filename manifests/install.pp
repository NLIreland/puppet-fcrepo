...
class fcrepo::install {
  ...
  tomcat::setenv::entry {'tomcat-fcrepo-catalina-opts':
    ...
    param       => 'CATALINA_OPTS',
    value       => "-Djava.awt.headless=true -Dfile.encoding=UTF-8 -server -Xmx${::fcrepo::tomcat_catalina_opts_xmx_real} -XX:MaxPermSize=${::fcrepo::tomcat_catalina_opts_maxpermsize_real} -XX:+UseG1GC -XX:+DisableExplicitGC -Xloggc:/tmp/java-gc.log -XX:+PrintGCDetails -XX:+PrintGCDateStamps -XX:+PrintGCTimeStamps -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/tmp/ -Djava.net.preferIPv4Stack=true -Djgroups.udp.mcast_addr=${::fcrepo::tomcat_catalina_opts_multicastaddr_real} -Dfcrepo.modeshape.configuration=file://${::fcrepo::fcrepo_configdir_real}/repository.json -Dfcrepo.jgroups.configuration=${::fcrepo::fcrepo_configdir_real}/jgroups-fcrepo-tcp.xml -Dfcrepo.mysql.host=${::fcrepo::fcrepo_db_host_real} -Dfcrepo.mysql.port=${::fcrepo::fcrepo_db_port_real} -Dfcrepo.mysql.username=${::fcrepo::fcrepo_db_username_real} -Dfcrepo.mysql.password=${::fcrepo::fcrepo_db_password_real}",
    quote_char  => "\"",
  }
